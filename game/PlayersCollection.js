...
PlayersCollection.prototype.removePlayer = function(socket, callback) {
	...
	try {
		if (p.player.isConnected) {
			...
			var partner = _.findWhere(this.players, {
				socket_id: p.player.partner_id
			}).player;
	...
	this.players = _.without(this.players, _.findWhere(this.players, {
		socket_id: socket.id
	}));
...
PlayersCollection.prototype.findPatner = function(socket, nickname, callback) {
	var player1 = _.findWhere(this.players, {
		socket_id: socket.id
	}).player;
